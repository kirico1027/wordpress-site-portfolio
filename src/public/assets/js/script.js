...
(function () {
  ...
  function initDownloadFormSample() {
    var form = document.querySelector(".download-form__form");
    if (!form || form.getAttribute("data-download-form-init") === "1") return;
    form.setAttribute("data-download-form-init", "1");

    var email = form.querySelector("#download-email");
    var emailConfirm = form.querySelector("#download-email-confirm");
    var emailError = form.querySelector("#download-email-confirm-error");
    var status = form.querySelector(".download-form__status");
    var mismatchMessage = "メールアドレスが一致していません。";
    var sampleMessage =
      "資料請求を受け付けました。\nこちらはポートフォリオ用フォームのため、実際の資料送付は行われていません。";
